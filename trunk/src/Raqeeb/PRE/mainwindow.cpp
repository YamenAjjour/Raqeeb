...
{
    addbounce* addb = new addbounce(0,this);
    QPair<int,int> data;
...
{
    addbounce* addb = new addbounce(1,this);
    QPair<int,int> data;
    if(addb->getData(data))
    {
        QString error;
...
{
    EmployeeInfo* e = new EmployeeInfo(this,userModel->item(i.row(),0)->data(Qt::UserRole).toInt());
        while(e->exec());
    QString error;
    if(e->getHasChanged())
        userModel->init(error);
