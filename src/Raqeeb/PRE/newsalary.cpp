...
void NewSalary::displayData()
{
    ...
    foreach(User u , users)
    {
        ...
        if(userModel->getUserInfo(u.getId(),info))
        {
            int hour = u.calcHour(ui->dFromDate->date(),ui->dToDate->date(),fogToSig,nightShift);
            forgData[u.getId()]=fogToSig;
            nightShiftData[u.getId()]=nightShift;
            QTableWidgetItem* idItem = new QTableWidgetItem(QString::number(u.getId()));
...
void NewSalary::showDetailedUserDialog(const QModelIndex &index)
{
    ...
    if(index.isValid())
    {
        ...
        int id =ui->tableWidget->item(index.row(),E_ID)->data(Qt::EditRole).toInt();
